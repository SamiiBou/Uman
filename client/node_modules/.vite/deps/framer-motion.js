...
function findSpring({ duration = springDefaults.duration, bounce = springDefaults.bounce, velocity = springDefaults.velocity, mass = springDefaults.mass }) {
  let envelope;
  let derivative;
  warning2(duration <= secondsToMilliseconds2(springDefaults.maxDuration), "Spring duration must be 10 seconds or less");
  let dampingRatio = 1 - bounce;
  dampingRatio = clamp(springDefaults.minDamping, springDefaults.maxDamping, dampingRatio);
  duration = clamp(springDefaults.minDuration, springDefaults.maxDuration, millisecondsToSeconds2(duration));
  if (dampingRatio < 1) {
  ...
  const generator = {
    ...
    next: (t) => {
      ...
      if (!isResolvedFromDuration) {
        ...
        if (dampingRatio < 1) {
          currentVelocity = t === 0 ? secondsToMilliseconds2(initialVelocity) : calcGeneratorVelocity(resolveSpring, t, current);
        }
...
var animateMotionValue = (name, value, target, transition = {}, element, isHandoff) => (onComplete) => {
  ...
  if (instantAnimationState.current || MotionGlobalConfig2.skipAnimations) {
    shouldSkip = true;
    options.duration = 0;
    options.delay = 0;
  }
...
function updateMotionValuesFromProps(element, next, prev) {
  for (const key in next) {
    ...
    if (isMotionValue(nextValue)) {
      ...
      if (true) {
        warnOnce2(nextValue.version === "12.6.3", `Attempting to mix Motion versions ${nextValue.version} with 12.6.3 may not work as expected.`);
      }
...
function refWarning(name, ref) {
  warning2(Boolean(!ref || ref.current), `You have defined a ${name} options but the provided ref is not yet hydrated, probably because it's defined higher up the tree. Try calling useScroll() in the same component as the ref, or setting its \`layoutEffect: false\` option.`);
}
...
function useSpring(source, config = {}) {
  ...
  const latestSetter = (0, import_react34.useRef)(noop2);
...
var WillChangeMotionValue = class extends MotionValue {
  ...
  add(name) {
    ...
    if (styleName) {
      addUniqueItem2(this.values, styleName);
      this.update();
    }
  }
  update() {
...
function useReducedMotion() {
  ...
  if (true) {
    warnOnce2(shouldReduceMotion !== true, "You have Reduced Motion enabled on your device. Animations may not appear as expected.");
  }
